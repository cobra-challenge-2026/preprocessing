...
def detect_posterior_surface(
    ...
    if separation_open_mm and separation_open_mm > 0:
        ...
        opened = sitk.BinaryMorphologicalOpening(body, radius, sitk.sitkBall, 0.0, 1.0)
    cc = sitk.RelabelComponent(sitk.ConnectedComponent(opened), sortByObjectSize=True)
    seed = sitk.Cast(cc == 1, sitk.sitkUInt8)
    if radius is not None:
        # the opening erodes/severs the patient's posterior surface (convex backs,
        # cushions); recover it by dilating the seed back into the original body.
        # a single bounded dilation can't bridge the severed couch gap (~2*radius),
        # so the couch stays excluded while the back contour is restored.
        seed = sitk.And(sitk.BinaryDilate(seed, radius, sitk.sitkBall), body)
    patient = sitk.GetArrayFromImage(seed) > 0
...
def generate_couch_masks_from_image(
    ...
    fov_mask: sitk.Image = None,
    body_threshold_hu: float = -300.0,
    surface_margin_mm: float = 5.0,
    flat_posterior: bool = True,
    posterior_percentile: float = 100.0,
